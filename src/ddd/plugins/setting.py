...
class SettingPlugin(PluginBase):
    ...
    def __init__(self):
        super().__init__(
            name="setting", summary="管理页面和插件的组织结构", category="system"
        )
    ...
    def _list_structure(self, renderer, structure, node_id: str) -> bool:
        ...
        node_name = (
            node_info.get("display_name", node_info.get("name", node_id))
            if node_info
            else node_id
        )

        renderer.print_banner(title="🌳 结构管理器", subtitle=f"节点: {node_name}")
    ...
    def _interactive_mode(self, renderer, structure, node_id: str) -> bool:
        ...
        while True:
            ...
            table = renderer.print_menu(
                title="管理操作", options=options, show_help=True
            )
            ...
            if choice == "q":
                ...
                target = self._get_target_input(
                    renderer, structure, node_id, "查看详情"
                )
                if target:
        ...
        while True:
            ...
            for i, child in enumerate(children, 1):
                ...
                selection_table.add_row(
                    f"[{i}]", f"{type_icon} {enabled_icon}", name, desc
                )
    ...
    def _view_item_details(
        self, renderer, structure, node_id: str, target: str
    ) -> bool:
        """查看项目详情"""
        children = structure.get_node_children(node_id)
        target_child = None
        ...
        details_table.add_row(
            "类型", "📄 页面" if target_child.get("type") == "page" else "🔌 插件"
        )
        details_table.add_row(
            "状态", "✅ 启用" if target_child.get("enabled", True) else "❌ 禁用"
        )
    ...
    def _get_target_input(
        self, renderer, structure, node_id: str, operation: str
    ) -> str:
        """获取目标项目输入"""
        children = structure.get_node_children(node_id)
        if not children:
    ...
    def _handle_reorder(self, renderer, structure, node_id: str) -> bool:
        ...
        try:
            ...
            if self._is_number_sequence(new_order_str):
                return self._reorder_by_numbers(
                    renderer, structure, node_id, new_order_str, children
                )
            else:
                # 按项目名称排序
                new_order = new_order_str.split()
                return self._reorder_items(
                    renderer, structure, node_id, {"items": new_order}
                )
